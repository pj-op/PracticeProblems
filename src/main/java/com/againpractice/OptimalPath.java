...
public class OptimalPath {
	 ...
	 * and desirable rocks just became available in New York, so you are planning a
	 * cross-country road trip. There are several other rare rocks that you could
	 ...
	 * Example:
	 * 
	 * { {0,0,0,0,5}, (New York) {0,1,1,1,0}, {2,0,0,0,0} (So_Cal) }
	 * 
	...
	/**
	 * @param A two dimensional integer array. This is a grid representing the all
	 *          the cities, and positions at which rocks are located along with the
	 *          number of rocks at each point.
	 * 
	 * @return An int givng the total number of rocks that can be collected along
	 *         the optimal path as per the rules specified above.
	 */
...
//			System.out.println(Arrays.toString(newGrid));
			System.out.println("========");
			Arrays.stream(newGrid).flatMap(arr -> Arrays.stream(arr).boxed()).forEach(System.out::println);
			System.out.println("========");
			return total += calculateOptimalPath(newGrid);
		} else {
