...
public class CheckIfEveryRowColumnContainsAllNumbers {
    public static void main(String[] args) {
        ...
        System.out.println(checkValidOptimal(matrix));
    }

    private static boolean checkValidOptimal(int[][] matrix) {
        int n = matrix.length;
        int[] freq = new int[n];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
    ...
    public static boolean checkValid(int[][] matrix) {
        int n = matrix.length;
        Map<Integer, Long> map = Arrays.stream(matrix)
                .flatMapToInt(Arrays::stream)
                .boxed()
                .collect(Collectors.groupingBy(num -> num, Collectors.counting()));

        for (Map.Entry<Integer, Long> entry : map.entrySet()) {
            if (entry.getValue() != n) return false;
        }
        return true;
    }
}
